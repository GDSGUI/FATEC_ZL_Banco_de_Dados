CREATE DATABASE Locacadora_Parte04
GO
USE Locacadora_Parte04
...
GO
SELECT DISTINCT Cl.Num_Cadastro, Cl.Nome, FM.Titulo, dvd.Data_Fabricacao,LC.Valor
from Cliente Cl, Filme FM, DVD dvd,Locacao LC
WHERE LC.DVD_Num = dvd.Num
	AND FM.ID = dvd.Filme_ID
		AND Cl.Num_Cadastro = LC.Cliente_Num_Cadastro
...
--de DVD's alugados por cliente (Chamar essa coluna de qtd), por data de locação
GO
SELECT DISTINCT CL.Num_Cadastro, CL.Nome,
CONVERT(CHAR(10),LC.Data_Locacao, 103) AS Datas,
COUNT(LC.DVD_Num) AS QTD
FROM Cliente CL, Locacao LC

GROUP BY CL.Num_Cadastro, CL.Nome, LC.DVD_Num, LC.Data_Locacao
ORDER BY CL.Nome
--3) Consultar, num_cadastro do cliente, nome do cliente, data de locação (formato DD/MM/AAAA) e o valor total de
--todos os dvd's alugados (Chamar essa coluna de valor_total), por data de locação
GO
SELECT DISTINCT CL.Num_Cadastro, CL.Nome,
CONVERT(CHAR(10),LC.Data_Locacao, 103) As Data_Locação,
SUM(LC.Valor) AS Valor_Total
FROM Cliente CL, Locacao LC,DVD dvd

WHERE CL.Num_Cadastro = LC.Cliente_Num_Cadastro
AND LC.DVD_Num = dvd.Num

GROUP BY CL.Num_Cadastro, CL.Nome, LC.Data_Locacao
--4) Consultar, num_cadastro do cliente, nome do cliente, Endereço concatenado de logradouro e numero como Endereço,
--data de locação (Formato DD/MM/AAAA) dos clientes que alugam mais de 2 filmes simultaneamente
GO
SELECT DISTINCT CL.Num_Cadastro, CL.Nome, CL.Logradouro + ' ,N°' + CAST(Num AS VARCHAR(10)) AS Endereço,
CONVERT(CHAR(10), LC.Data_Locacao, 103) AS Data_Locação,
COUNT(LC.Data_Locacao) AS QTD
FROM Cliente CL, Locacao LC

WHERE CL.Num_Cadastro = LC.Cliente_Num_Cadastro

GROUP BY CL.Num_Cadastro, CL.Nome, CL.Logradouro, CL.Num ,LC.Data_Locacao, CL.Num_Cadastro
HAVING COUNT((LC.Data_Locacao)) > 2